...
# Access the run
run = api.run('lucabaroni/sweep_LSV1M/a3bxjex0')
...
for artifact in artifacts:
    ...
    artifact_dir = artifact.download()  # Downloads the artifact and returns the directory
    print(f"Artifact downloaded to: {artifact_dir}")

    x= torch.load(os.path.join(artifact_dir, 'core_sd.pth'))
...
# dataloaders = get_LSV1M_dataloaders(population='both', dataloaders_batch_size=300, dataloaders_n_images_val=5000, neurons_subset='0')
# %%
stims_c = stims-(stims.max() + stims.min())/2
stims_multi_c = stims_multi-(stims_multi.max() + stims_multi.min())/2

ds_train =  LSV1M_Dataset(resps = resps[:-n_images_val, neuron_idxs], stims = stims_c[:-n_images_val])
ds_validation = LSV1M_Dataset(resps = resps[-n_images_val:, neuron_idxs], stims = stims_c[-n_images_val:])
...
ds_test =  LSV1M_Dataset(resps = resps_multi.mean(axis=1)[:, neuron_idxs], stims = stims_multi_c)
dataloaders_c['test'] =  {'all_sessions': DataLoader(ds_test, batch_size=16, shuffle=True, num_workers=0)}

results = {
    # 'val/corr' : output['validation_corr'], 
    # 'val/MSE' : get_MSE(model, dataloaders['validation'], as_dict=False, per_neuron=False), 
    # 'test/MSE': get_MSE(model, dataloaders['test'], as_dict=False, per_neuron=False),
    'val/corr': get_correlations(model, dataloaders_c['validation'], as_dict=False, per_neuron=False),
    'test/avg_corr': get_correlations(model, dataloaders_c['test'], as_dict=False, per_neuron=False),
    # 'test/avg_corr': get_avg_correlations(model, dataloaders['test'], as_dict=False, per_neuron=False),
}
print(results)
